...
optimize(const string &pwsFileName)
{
    ...
    if (zeroPositions[0].size() < 1) {
        return;
    } else if (zeroPositions[0].size() > 1) {
    ...
    for (unsigned layN = 1; (layN + 1) < circuitDesc.size(); ) {
        ...
        if (prevDeletes.size() != 0) {
            auto pDelBegin = prevDeletes.begin();
            // first, renumber gates at this layer
            for (unsigned gateN = 0; gateN < layer.size(); gateN++) {
                GateDescription &gate = layer[gateN];
                gate.in1 -= distance(pDelBegin, prevDeletes.lower_bound(gate.in1));
                gate.in2 -= distance(pDelBegin, prevDeletes.lower_bound(gate.in2));
            }
            finalZeros[layN-1] -= distance(pDelBegin, prevDeletes.lower_bound(finalZeros[layN-1]));

            // next, delete all gates at previous layer
            for (auto rit = prevDeletes.rbegin(); rit != prevDeletes.rend(); rit++) {
                prevLayer.erase(prevLayer.begin() + *rit);
            }

            // and finally, update the GatePosition information in all gates at previous layer
            for (unsigned gateN = 0; gateN < prevLayer.size(); gateN++) {
                prevLayer[gateN].pos.name = gateN;
            }
        }
